class Image{
  ...
  widthAllImgsForMinHeight(){
    ...
    return (this.widthRowForHeight(this.heightRowMin)+this.wPreviewForMinHeight);
...
class Gallery {
  ...
  get imagesOfLastRow(){
    ...
    return ((this.listImages.length>0) ? this.listImages.filter(img => img.row === this.lastRow) : []);
  ...
  get widthRow(){
    ...
    if (innerWidth<this.widthRowMin){res=this.widthRowMin}

    if ((innerWidth>=this.widthRowMin)&(innerWidth<=this.widthRowMax)){res=innerWidth-1}

    if (innerWidth>this.widthRowMax){res=this.widthRowMax}
  ...
  widthLastRowForHeight(height_row){
    ...
    if (listImages.length>0) {
      ...
      listImages.forEach( img => { widthImages +=  height*img.width/img.height; });
...
export const PreviewSize = (listImages,img ,innerWidth, sett)  => {
  ...
  if (((gllr.widthAllImgsForMinHeight+image.wPreview)<=gllr.widthRow)&&((gllr.widthAllImgsForMaxHeight+image.wPreview)>gllr.widthRow)) 
